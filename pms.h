...
template<class T> void ISDist::print_array(ofstream& ofs, vector<T>& vec, string name) {
    if (!vec.empty()) {
        ofs << "    \"" << name << "\": [" << vec[0];
...
void ISDist::print_best_solution()
{
    ...
    ofstream ofs(name +".json", ios::app);
    ofs << endl;
    ofs << "    {" << endl;
    ofs << R"(        "name": ")" << addr << "\"," << endl;
    if (best_soln_feasible == 0) {
        ofs << R"(        "obj": "-", )" << endl;
        ofs << R"(        "opt_time": "-", )" << endl;
    } else {
        ofs << R"(        "obj": )" << opt_unsat_weight << "," << endl;
    ...
    ofs << R"(        "num_local_optima": )" << num_local_optima << "," << endl;
    ...
    ofs << R"(        "propagate_succ_len": )" << propagate_succ_len << "," << endl;
