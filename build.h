...
void ISDist::build_instance(char *filename)
{
    ...
    while (getline(infile, line))
    {
        ...
        if (clause_reduent == 1)
        {
            ...
            num_clauses--;
            if (org_clause_weight[c] == top_clause_weight) { num_hclauses--; }
            else num_sclauses--;
            clause_lit_count[c] = 0;
            continue;
        }
    ...
    if (num_hclauses + num_sclauses < num_clauses) {
...
void ISDist::allocate_memory()
{
    ...
    cur_soln_cpy = new int[malloc_var_length];
    ...
    propagate_table.resize(2 * malloc_var_length);
    flag.resize(2 * malloc_var_length, imply_level::empty);
    propagate_record.init(malloc_var_length);
}
